...
public class HomeActivity extends BaseActivity implements NavigationBarView.OnItemSelectedListener {
    ...
    private void initConfig() {
        // WallConfig/LiveConfig/VodConfig 的 init() 方法内部会执行 Room 数据库查询 (Config.wall/live/vod())
    ...
    private Callback getCallback() {
        return new Callback() {
            ...
            public void success(String result) {
                android.util.Log.d("HomeActivity_DBG", "callback success with msg: " + result);
                Notify.show(result);
            }

            @Override
            public void success() {
                android.util.Log.d("HomeActivity_DBG", "callback success, firing RefreshEvent");
            ...
            public void error(String msg) {
                android.util.Log.d("HomeActivity_DBG", "callback error: " + msg);
                RefreshEvent.config();
                StateEvent.empty();
                Notify.show(msg);
            }
    ...
    private void setNavigation() {
        ...
        // LiveConfig.hasUrl() 内部会调用 Config.live() 执行 Room 数据库查询
        // 在 initConfig() 执行前调用会导致主线程数据库访问异常 (IllegalStateException)
        // 初始默认隐藏直播标签，配置加载完成后 onRefreshEvent 会再次调用 setNavigation() 更新可见性
