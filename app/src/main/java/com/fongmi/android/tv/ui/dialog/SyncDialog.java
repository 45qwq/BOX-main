...
public class SyncDialog extends BaseDialog implements DeviceAdapter.OnClickListener, ScanTask.Listener {
    ...
    public void onItemClick(Device item) {
        App.execute(() -> {
            FormBody requestBody = buildBody();
            App.post(() -> OkHttp.newCall(client, String.format(Locale.getDefault(), "%s/action?do=sync&mode=%s&type=%s", item.getIp(), binding.mode.getTag().toString(), type), requestBody).enqueue(getCallback()));
        });
    }
    ...
    public boolean onLongClick(Device item) {
        ...
        App.execute(() -> {
            ...
            FormBody requestBody = buildBody();
            App.post(() -> OkHttp.newCall(client, String.format(Locale.getDefault(), "%s/action?do=sync&mode=%s&type=%s&force=true", item.getIp(), binding.mode.getTag().toString(), type), requestBody).enqueue(getCallback()));
        });
        return true;
    }
