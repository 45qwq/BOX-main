...
public class DownloadAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private final List<Object> displayItems; // Download 或 DownloadGroup
    private TabMode tabMode = TabMode.ACTIVE;

    public enum TabMode {
        ACTIVE,     // 下载中（flat list）
        COMPLETED   // 已完成（分组列表）
    }
    ...
    /**
     * 刷新列表
     * @param activeList 下载中的任务列表
     * @param completedList 已完成的任务列表
     */
    ...
    private void smartUpdateFlat(List<Download> newList) {
        ...
        // 完全重建列表，用 notifyDataSetChanged 避免逐项动画闪烁
        List<Object> rebuilt = new ArrayList<>(newList);
        displayItems.clear();
        displayItems.addAll(rebuilt);
        notifyDataSetChanged();
    }
    ...
    class ItemViewHolder extends RecyclerView.ViewHolder {
        ...
        private void updateStatus(Download item) {
            ...
            if (Status.COMPLETED.name().equals(status) || item.getStatusInt() == Download.STATUS_COMPLETED) {
            ...
            } else if (Status.MERGING.name().equals(status)) {
                ...
                binding.action.setImageResource(R.drawable.ic_action_delete);
            } else if (Status.QUEUED.name().equals(status)) {
                binding.status.setText("排队中");
                binding.progress.setProgress(0);
                binding.progressText.setText("等待下载");
                binding.action.setImageResource(R.drawable.ic_action_delete);
            } else if (Status.FAILED.name().equals(status) || item.getStatusInt() == Download.STATUS_FAILED) {
