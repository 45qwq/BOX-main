...
public class Notify {
    ...
    public static void show(Notification notification) {
        if (Build.VERSION.SDK_INT >= 33 && ActivityCompat.checkSelfPermission(App.get(), Manifest.permission.POST_NOTIFICATIONS) != PackageManager.PERMISSION_GRANTED) {
            CharSequence text = notification.extras == null ? null : notification.extras.getCharSequence(Notification.EXTRA_TITLE);
            show(text != null ? text.toString() : "通知权限未开启");
            return;
        }
