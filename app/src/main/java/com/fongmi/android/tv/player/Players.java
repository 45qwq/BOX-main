...
public class Players implements Player.Listener, ParseCallback {
    ...
    private void setMediaItem(Map<String, String> headers, String url, String format, Drm drm, List<Sub> subs, List<Danmaku> danmakus, long timeout) {
        this.url = url;
        Uri mediaUri = url.startsWith("file://") || url.startsWith("/") ? FileUtil.getShareUri(url) : UrlUtil.uri(url);
        if (exoPlayer != null) exoPlayer.setMediaItem(ExoUtil.getMediaItem(this.headers = checkUa(headers), mediaUri, this.format = format, this.drm = drm, checkSub(this.subs = subs), decode));
