...
public class HistoryDialog implements ConfigAdapter.OnClickListener {
    ...
    private void loadConfigs() {
        App.execute(() -> {
            List<Config> all = Config.getAll(type);
            Config current = type == 0 ? VodConfig.get().getConfig() : WallConfig.get().getConfig();
            List<Config> items = new ArrayList<>();
            for (Config config : all) {
                if (config.equals(current) || config.isEmpty()) continue;
                items.add(config);
            }
            App.post(() -> {
