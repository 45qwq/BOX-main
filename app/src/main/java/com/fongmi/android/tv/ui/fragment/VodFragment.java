...
public class VodFragment extends BaseFragment implements SiteCallback, FilterCallback, TypeAdapter.OnClickListener, ConfigCallback {
    ...
    private TypeFragment getFragment() {
        return ((PageAdapter) mBinding.pager.getAdapter()).getFragment(mBinding.pager.getCurrentItem());
    }
    ...
    protected void initData() {
        // 懒加载非关键数据：热点数据、网络请求、Room 读取，不阻塞首次渲染
        initHot();
        getHot();
        // 检查是否需要显示上次播放弹窗
    ...
    private void initStartupState() {
        // 必须在后台线程读取配置（Room 禁止主线程访问），结果回主线程设置 UI
        App.execute(() -> {
            ...
            boolean finalHasExistingConfig = hasExistingConfig;
            App.post(() -> applyStartupState(finalHasExistingConfig));
        });
    }

    private void applyStartupState(boolean hasExistingConfig) {
    ...
    private void checkLastWatchDialog() {
        if (App.isAppJustLaunched()) {
            // Room 查询异步化，避免主线程 I/O
            App.execute(() -> {
                List<History> histories = History.get();
                App.post(() -> {
                    if (!histories.isEmpty()) {
                        // 检查 Activity 是否仍处于前台运行状态，防止 BadTokenException
                        Activity activity = getActivity();
                        if (activity != null && isAdded() && !activity.isFinishing() && !activity.isDestroyed()) {
                            LastWatchToast.create(activity, histories.get(0)).show();
                        }
                    }
                    App.setAppLaunched();
                });
    ...
    protected void onPickFile(@Nullable Uri uri) {
        if (uri == null) return;
        VideoActivity.file(getActivity(), FileChooser.getPathFromUri(getContext(), uri));
    }

    @Override
    public boolean canBack() {
        if (mBinding.pager.getAdapter() == null) return true;
        if (mBinding.pager.getAdapter().getItemCount() == 0) return true;
    ...
    class PageAdapter extends FragmentStateAdapter {
        ...
        private final SparseArray<TypeFragment> fragments = new SparseArray<>();

        public PageAdapter(@NonNull Fragment fragment) {
            super(fragment);
        }

        public TypeFragment getFragment(int position) {
            TypeFragment f = fragments.get(position);
            if (f == null) f = (TypeFragment) createFragment(position);
            return f;
        }
        ...
        public Fragment createFragment(int position) {
            ...
            TypeFragment f = TypeFragment.newInstance(getSite().getKey(), type.getTypeId(), type.getStyle(), type.getExtend(true), "1".equals(type.getTypeFlag()));
