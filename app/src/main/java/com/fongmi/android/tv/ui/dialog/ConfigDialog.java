...
public class ConfigDialog {
    ...
    private void initDialog() {
        dialog = new MaterialAlertDialogBuilder(binding.getRoot().getContext()).setTitle(type == 0 ? R.string.setting_vod : R.string.setting_wall).setView(binding.getRoot()).setPositiveButton(edit ? R.string.dialog_edit : R.string.dialog_positive, this::onPositive).setNegativeButton(R.string.dialog_negative, this::onNegative).create();
    ...
    private void onPositive(DialogInterface dialog, int which) {
        ...
        // 如果URL为空，删除配置
        if (url.isEmpty()) {
            // Config.delete 是 Room 数据库操作，必须在后台线程执行
            String finalOri = ori;
            App.execute(() -> Config.delete(finalOri, type));
            dialog.dismiss();
            return;
        }
        ...
        // Config.find / Config.delete 等 Room 数据库操作必须在后台线程执行
        App.execute(() -> {
            // 如果是编辑模式，更新现有配置
            if (isEdit) Config.find(finalOri, finalType).url(finalUrl).name(finalName).update();

            Config config = Config.find(finalUrl, finalType);

            // 回调到主线程（callback.setConfig 可能涉及 UI 操作）
            App.post(() -> {
                callback.setConfig(config);

                // 添加一个延迟检查，如果配置没有成功加载，则恢复原始URL
                new android.os.Handler().postDelayed(() -> {
                    App.execute(() -> checkConfigLoad(finalUrl, originalUrl, finalType));
                }, 2000);
    ...
    /**
     * 检查配置是否成功加载，失败时恢复原始URL（在后台线程调用）
     */
    private void checkConfigLoad(String url, String originalUrl, int type) {
        Config currentConfig = getConfig();
        if (currentConfig == null || !currentConfig.getUrl().equals(url)) {
            if (!TextUtils.isEmpty(originalUrl)) {
                Config config = Config.find(originalUrl, type);
                App.post(() -> callback.setConfig(config));
            } else {
                restoreEmptyConfig(type);
            }
        }
    }
     ...
     * 恢复为空配置（在后台线程调用）
     */
    private void restoreEmptyConfig(int type) {
        switch (type) {
            case 0:
