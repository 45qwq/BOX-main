...
├── danmaku/            # 弹幕引擎
├── forcetech/          # ForceTech 视频提取（P2P/P3P/MITV）
├── hook/               # Hook 功能模块（签名伪装）
└── jianpian/           # 简片视频提取
```
> 注：投屏（DLNA）功能通过 `app/libs/` 下的 Cling 2.1.1 本地 jar 实现，源码位于 `app/src/main/java/.../utils/` 的 `UpnpCastManager`、`CastUpnpService` 等，不再单独成模块。
